...
export class MatchListComponent {
  ...
  isPastMatch(match: Match): boolean {
    ...
    // Matches start at 7:30 PM IST (14:00 UTC).
    // We add a 6-hour buffer to allow for rain/weather delays (ending at 1:30 AM IST next day / 20:00 UTC).
    const matchEndDate = new Date(Date.UTC(year, month, day, 20, 0, 0));
    const now = new Date();

    return now > matchEndDate;
  }
